...
//				{
//					CH_N[i] = 1.2f *((s16)Rc_Ppm_In[i] - 1100); //700 -- 1500us,处理成+-500摇杆量
//				}
//				else
//				{
//					CH_N[i] = 0;
...
//			}		
//		}
//        else if(Ano_Parame.set.pwmInMode==DBUS)//{
              for(u8 i=0;i<6;++i){
                  CH_N[i]=0.76f*((s16)(Rc_dbus_In[i])-1024);   //364-1684处理成-500~+500摇杆量
                  CH_N[i] = LIMIT(CH_N[i],-500,500);//限制到+—500
              }      
...
{
    if(Ano_Parame.set.pwmInMode!=DBUS){
    static u16 cnt;
	static s8 cnt2;
	cnt += dT_ms;
	if(cnt >= 500) //500*dT 秒
	{
		cnt=0;
		if((chn_en_bit & 0x0F) != 0x0F || flag.chn_failsafe ) //前4通道有任意一通道无信号
		{
			cnt2 ++;
		}
		else
